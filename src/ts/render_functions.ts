...
/* RENDERING
============ */
export function get_projection(fov:number):mat4 {
	const aspect:number = context.screen_dims[0] / context.screen_dims[1];
...
function set_textures(texture_map:Array<[number,WebGLTexture]>):void {
	for(let i=0; i<texture_map.length; i++) {
		gl.activeTexture(gl.TEXTURE0+i);
		gl.bindTexture(gl.TEXTURE_2D, texture_map[i][1]);
		gl.uniform1i(texture_map[i][0], i);
	}
}

/* SHADOW MAPPING PASS
====================== */
export function shadowmap_pass(room:Room, light:Spotlight):void {
	// bind fb
	...
	// use shader
	let shader = context.shaders.shadowmap_pass;
	...
	// set textures
	set_textures([shader.uniforms.sm_tex,pd.tx.sm_linear_generic]);
	...
	// set textures
	gl.activeTexture(gl.TEXTURE0);
	gl.bindTexture(gl.TEXTURE_2D, pd.tx.sm_cache[0]);
	gl.uniform1i(shader.uniforms.sm_tex, 0);
...
==================================== */
export function gbuffer_pass(gl:any, pd:any, proj_m:mat4):void {
	// Bind to deferred fb
...
/* SSAO PASS
==================== */
export function ssao_pass(gl:any, pd:any, proj_m:mat4):void {
	// set fbo
	...
	gl.uniformMatrix4fv(
		...
		false,
		proj_m);
	gl.uniform2f(shader.uniforms.ssao_dims, gl.canvas.width, gl.canvas.height);
...
/* SSAO BLUR
==================== */
export function ssao_blur(gl:any, pd:any, t3:vec3):void {
	// set fbo
...
/* SPOTLIGHT INT PASS
===================== */
export function spotlight_pass(gl:any, pd:any, room:Room, t3:vec3):void {
	// set fbo
	...
	// global uniform set
	// ------------------
	gl.uniform2f(shader.uniforms.screen_dims, gl.canvas.width, gl.canvas.height);
	gl.uniform3fv(shader.uniforms.time, t3);
...
================================== */
export function quad_deferred_combine(gl:any, pd:any, write_to_frame:boolean, t3:vec3):void {
	// set fbo
...
/* FXAA PASS
==================== */
export function fxaa_pass(gl:any, pd:any):void {
	// set fbo
...
======================== */
export function render(gl:any, pd:any, t:number):void {
	// time adjustments
